...
class Krotov_optimizer_CCNOT_Shorten():
    def __init__(self,N_Iteration=100,N_T=55):
        self.dirpath = os.path.dirname(__file__)
        self.datapath = self.dirpath + os.sep + "data_CCNOT_" + str(N_T) + "_short.pickle"
        ...
        self.control_switch = np.ones((4,self.N_T))

        self.mute_memory = 2*np.ones((4,self.N_T)) # 2: unexplored, 1: explored but not deleted, 0: explored and then deleted
        self.mute_memory_data = {"mute_memory":self.mute_memory}
        self.mute_memory_path = self.dirpath + os.sep + "mute_memory_data.pickle"
    ...
    def mute_initial(self,mode=0):
        if mode == 0:
            for n in range(self.N_T):
                if n % 2 == 0:
                    self.control_switch[0][n] = 0
                    self.mute_memory[0][n] = 0
        else:
            with open(self.mute_memory_path,"rb") as f:
                self.mute_memory_data = pickle.load(f)
            self.mute_memory = self.mute_memory_data["mute_memory"]
            for i in range(4):
                for j in range(self.N_T):
                    if self.mute_memory[i][j] == 0:
                        self.control_switch[i][j] = 0

    def mute_control(self,time_index=0,control_index=0):
        self.control_switch[control_index][time_index] = 0
        if time_index % 2 == 0:
            if control_index == 0:
                self.J_12[time_index] = 0
            elif control_index == 1:
                self.J_34[time_index] = 0
            elif control_index == 2:
                self.J_56[time_index] = 0
            else:
                self.J_78[time_index] = 0
        else:
            if control_index == 0:
                self.J_23[time_index] = 0
            elif control_index == 1:
                self.J_45[time_index] = 0
            elif control_index == 2:
                self.J_67[time_index] = 0
            else:
                self.J_89[time_index] = 0
    ...
    def _sum_bpf(self,backward,partial,forward):
        s = 0
        for k in range(self.N_k):
            s += np.imag((((backward[k].conj().trans())*(partial)*forward[k]).full())[0][0])
        return s
    ...
    def optimize(self):
        ...
        for i in range(self.N_Iteration):
            ...
            for n in range(self.N_T):
                ...
                if n % 2 == 0:
                    self.J_12[n] += self.control_switch[0][n]*(self.S_12[n]/self.step_12)*self._sum_bpf(backward,self.J0*self.H_12,forward)
                    self.J_34[n] += self.control_switch[1][n]*(self.S_34[n]/self.step_34)*self._sum_bpf(backward,self.J0*self.H_34,forward)
                    self.J_56[n] += self.control_switch[2][n]*(self.S_56[n]/self.step_56)*self._sum_bpf(backward,self.J0*self.H_56,forward)
                    self.J_78[n] += self.control_switch[3][n]*(self.S_78[n]/self.step_78)*self._sum_bpf(backward,self.J0*self.H_78,forward)
                else:
                    self.J_23[n] += self.control_switch[0][n]*(self.S_23[n]/self.step_23)*self._sum_bpf(backward,self.J0*self.H_23,forward)
                    self.J_45[n] += self.control_switch[1][n]*(self.S_45[n]/self.step_45)*self._sum_bpf(backward,self.J0*self.H_45,forward)
                    self.J_67[n] += self.control_switch[2][n]*(self.S_67[n]/self.step_67)*self._sum_bpf(backward,self.J0*self.H_67,forward)
                    self.J_89[n] += self.control_switch[3][n]*(self.S_89[n]/self.step_89)*self._sum_bpf(backward,self.J0*self.H_89,forward)
            ...
            tau = 0
            for k in range(self.N_k):
                tau += np.real((((self.target_state[k].conj().trans())*forward_propagator[k][self.N_T]).full())[0][0])
            self.JT_final[i] = 1-(tau/self.N_k)

            if i < 10 or (i+1) % 100 == 0:
                print("Iteration=%d, JT=%.8f" % tuple([i+1,self.JT_final[i]]))
            
            if self.JT_final[i] < 10**(-8):
                break
    ...
    def save_memory(self):
        self.mute_memory_data = {"mute_memory":self.mute_memory}
        with open(self.mute_memory_path,"wb") as f:
            pickle.dump(self.mute_memory_data,f)
